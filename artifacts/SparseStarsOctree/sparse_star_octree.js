...
export class SparseStarOctreeBuilder {
    ...
    build() {
        ...
        this.flatNodes = [];
        this._subdivide(this.stars, cx, cy, cz, halfSize, 0);
     ...
     * @param {number} depth
     * @returns {number} Index of the built node in flatNodes
     */
    // FIXED
     ...
     * @param {number} childrenStartIndex
     */
    _computeMacroSphereFromChildren(parent, childrenStartIndex) {
        let totalMass = 0;
        let weightedX = 0, weightedY = 0, weightedZ = 0;
        let totalR = 0, totalG = 0, totalB = 0;

        for (let i = 0; i < 8; i++) {
            const child = this.flatNodes[childrenStartIndex + i];
