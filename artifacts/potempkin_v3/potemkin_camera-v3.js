...
export function updateCameraA(state) {
    ...
    // Construct view matrix: [R_x, U_x, F_x, 0, R_y, U_y, F_y, 0, R_z, U_z, F_z, 0, -dot(R,eye), -dot(U,eye), -dot(F,eye), 1]
...
/**
 * Multiplication of two quaternions: r = a * b (Corrected memory layout leaks)
...
/**
 * Converts a 3x3 orthonormal basis matrix into a unit quaternion.
...
export function basisToQuaternion(right, up, forward, layout) {
    ...
    const r01 = up[0], r11 = up[1], r21 = up[2];
    const r02 = forward[0], r12 = forward[1], r22 = forward[2];
...
/**
 * Synchronizes Camera A state (spherical) to Camera B state (quaternion).
 * @param {CameraState} state 
 */
...
/**
 * Synchronizes Camera B state (quaternion) to Camera A state (spherical).
 * @param {CameraState} state 
 */
