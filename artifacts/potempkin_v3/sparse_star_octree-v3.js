...
export class SparseStarOctreeBuilder {
    ...
    _flattenTree(node) {
        ...
        // Construct the flat representational node
        /** @type {OctreeNode} */
        const flatNode = {
            cx: node.cx,
            cy: node.cy,
        ...
        if (!node.isLeaf && node.children) {
            // Allocate 8 contiguous slots immediately following this node
            const childrenStartIndex = this.flatNodes.length;
            flatNode.childrenPtr = childrenStartIndex;

            for (let i = 0; i < 8; i++) {
                this.flatNodes.push(/** @type {any} */({})); // placeholders
            }

            // Recursively flatten each child and populate back into the correct contiguous slots
            for (let i = 0; i < 8; i++) {
                const childNode = node.children[i];
                const childFlatIdx = this.flatNodes.length;
                this._flattenTree(childNode);
                
                // Copy the flattened node details into its allocated contiguous slot
                this.flatNodes[childrenStartIndex + i] = this.flatNodes[childFlatIdx];
                
                // Pop the redundant trailing copy produced by the recursion
                this.flatNodes.pop();
            }
        }
    }
