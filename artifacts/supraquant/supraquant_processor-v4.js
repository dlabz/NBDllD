 ...
 * Now strictly periodic across [0, 2pi] by normalizing theta at the entry point of the recursion.
 * @param {number} i - Basis index
 ...
 * Now enforces a strict mathematical partition of unity to restore affine invariance.
    ...
    for (var r = 1u; r <= 3u; r = r + 1u) {
        ...
        for (var i = start_idx; i <= 3u; i = i + 1u) {
            ...
            if (abs(denom1) > 1e-6 && i > start_idx) {
                term1 = (sin((norm_theta - (*knots)[knot_idx]) * scale_factor) / denom1) * b[i - 1u];
            }
            if (abs(denom2) > 1e-6 && i < 3u) {
                term2 = (sin(((*knots)[knot_idx + r + 1u] - norm_theta) * scale_factor) / denom2) * b[i];
            }
