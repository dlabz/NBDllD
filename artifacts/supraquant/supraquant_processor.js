 ...
 * @returns {number}
 */
export function evaluateSupraQuantBasis(i, p, theta, knots) {
    if (p === 0) {
    ...
    if (Math.abs(sinDenom1) > 1e-6) {
        term1 = (Math.sin(theta - knots[i]) / sinDenom1) * evaluateSupraQuantBasis(i, p - 1, theta, knots);
    }
    if (Math.abs(sinDenom2) > 1e-6) {
        term2 = (Math.sin(knots[i + p + 1] - theta) / sinDenom2) * evaluateSupraQuantBasis(i + 1, p - 1, theta, knots);
    }
...
export function generateBoundaryLoop(numSamples, controlPoints, useSupraQuant) {
    ...
    // Cubic degree B-spline requires degree + numCp + 1 knots.
    // For a closed circle, we wrap knots around 2*PI boundary.
    const p = 3;
    ...
    for (let s = 0; s < numSamples; s++) {
        ...
        for (let i = 0; i < numCp; i++) {
            const basis = useSupraQuant 
                ? evaluateSupraQuantBasis(i, p, theta, knots)
        ...
        // This is the model representation S(theta) = Sum N(theta) * M(theta) * P_i * Scale
    ...
    for (let j = 0; j < numSamples; j++) {
        ...
        // 4th derivative central difference weights: [1, -4, 6, -4, 1] / h^4
...
fn evaluate_g4_basis(i: u32, p: u32, theta: f32, knots: ptr<function, array<f32, 12>>) -> f32 {
    if (p == 0u) {
    ...
    if (abs(sin_denom1) > 1e-6) {
        term1 = (sin(theta - (*knots)[i]) / sin_denom1) * evaluate_g4_basis(i, p - 1u, theta, knots);
    }
    if (abs(sin_denom2) > 1e-6) {
        term2 = (sin((*knots)[i + p + 1u] - theta) / sin_denom2) * evaluate_g4_basis(i + 1u, p - 1u, theta, knots);
    }
...
fn expand_supraquant_vertex(base_pos: vec3f, theta: f32, scale: f32) -> vec3f {
    ...
    for (var i = 0u; i < 8u; i = i + 1u) {
        let basis = evaluate_g4_basis(i, 3u, theta, &knots);
        rx = rx + basis * cp[i].x;
        ry = ry + basis * cp[i].y;
    }
